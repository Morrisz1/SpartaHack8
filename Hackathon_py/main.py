...
def main():
    print("DO NOT TAKE ANY ADVICE GIVEN BY THIS PROGRAM")
    print("Older Data:")
    fname_A = input()
    print("Newer Data:")
    fname_B = input()
...
def compare_data_draw(df_1, df_2):
    ...
    ax = df_1.plot(x='Minutes', y='Older')
    df_2.plot(ax=ax, x='Minutes', y='Newer', kind='line')
    plt.show()
...
def compare_data(df_1, df_2, target):
    ...
    if len(dp_1) == len(dp_2):
        ...
        print("The smallest difference was {} at minute {}".format(diff[0][0], diff[0][1]))
        print("The biggest difference was {} at minute {}".format(diff[-1][0], diff[-1][1]))
        print("Your BG has changed on average by {}".format(avg_diff))
        target_diff = avg - target*1.3
        if target_diff > 7:
            print("I recommend increasing your insulin ratio")
        elif target_diff < -5:
            print("I recommend lowering your insulin ratio")
        else:
            print("I do not recommend changing your insulin ratio")
